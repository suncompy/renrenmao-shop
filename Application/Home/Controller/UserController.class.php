...
class UserController extends HomeController{
  ...
  public function register($username = '', $password = '', $repassword = '', $verify = '', $randcode = ''){
    ...
    if(IS_POST){ //注册用户
      ...
      if(I('union_code')){
        if(!$my_union = union_code(I('union_code') , 1 )){
          $this->error('该推广码无效，或留空去个人中心完善');
        }
        if(!M('User')->where('id ='.$my_union.' and status = 1')->find()){
          $this->error('该推广用户或被禁用');
        }
      }
      if(C('REGISTER_VERIFY_VALID') && !check_verify(I('verify'), 1, false)){
        $this->error('验证码错误！');
      }
      if(!$username){
        $this->error('账号不能为空！');
      ...
      //   //登录接口
      //   $url = $domain . '?api_type=jipu&mod=Jipu&act=register&phone=' . $username . '&username=' . $username . '&password=' . $password;
      //   $rs = file_get_contents($url);
      //   $rs = json_decode($rs, true);
      //   if ($rs['status'] == 0) $this->error($rs['msg']);
      //   //$username = 'jipu_' . $username;
      // }
      //EDU系统用户注册 buddha 20170313 start-------------------
      // if (C('EDU_LOGIN')) {
      //   $domain = C('EDU_LOGIN_DOMAIN');
      //   //登录接口
      //   $jiami_pwd = jiami($password,'eduline');
      //   $usid = M('User')->where('1=1')->max('id');
      //   $usid = $usid + 1;
      //   $url = $domain . '?app=api&mod=Jipu&act=register&usid='.$usid.'&phone=' . $username . '&username=' . $username . '&password=' . $jiami_pwd;
      //   $rs = file_get_contents($url);
      //   $rs = json_decode($rs, true);
      //   if ($rs['code'] == 0) $this->error($rs['msg']);
      // }
      //EDU系统用户注册 buddha 20170313 end---------------------
      
      //防止页面一直重复停留在这
      if(strrpos(Cookie('__forward__') , '/User/' ) > 0){
        Cookie('__forward__' , U('Index/index'));
      }
      //调用注册接口注册用户 
      ...
      if(0 < $uid){
        ...
        if(0 < $uid){ //UC登录成功
          ...
          if($member_model->login($uid)){ //登录用户
            //登录后更新用户头像和昵称 TODO:
            $session_user = session('user_auth');
            $member_data = array(
              'uid' => $uid,
              'nickname' => I('post.nickname') ? : $session_user['username'],
      ...
      //TODO：读取用户协议，请阅读并同意用户协议
      $data = M('ArticleSystem')->where(array('code'=>'user_agreement'))->find();
  ...
  public function login($username = '', $password = '', $remember = ''){
    ...
    if(IS_POST){ //登录验证
      ...
      if(0 < $uid){ //UC登录成功
        ...
        if($member_model->login($uid)){ //登录用户
          ...
          if($remember == 1){
            ...
            $auto_uid = think_encrypt(SITE_URL.'.'.$uid);
            //将remember_data存入数据库
            $auto_data = array(
              'uid' => $uid,
            ...
            Cookie('__auto_uid__', $auto_uid, C('USER_AUTO_LOGIN_DAYS') * 24 * 3600 );
          }
          if ( (C('EDU_LOGIN') != 0) && C('EDU_LOGIN_DOMAIN')) {
            // 登陆成功之后，EDU处理
            $_now = date('Ymd');
      ...
      }elseif(is_weixin() && is_mobile() && !(strpos(SITE_DOMAIN, '192.168.') > -1)){
  ...
  public function getRandCode($type = ''){
      ...
      if($type == 'withdrawbind'){
        if(!$verify)$verify = I('get.verify');
          //绑定银行卡时，先验证验证码是否正确
          $verify_status = check_verify($verify, $id = 1 ,false);
          if(!$verify_status){
              $this->ajaxReturn(['status'=>0,'info' => '验证码输入错误！']);
          }
      }
      $sendTime = session('msg_'.$type);
      $limiTime = 60;
      if(isset($sendTime) && !empty($sendTime) && ($sendTime+$limiTime > NOW_TIME)){
          $this->ajaxReturn(['status'=>0,'info' => '请勿频繁操作！']);
      }
      $res_data = A('User', 'Event')->getRandCode($type);
      if($res_data['status']){
          session('msg_'.$type,NOW_TIME);
      }
  ...
  public function checkUserInfo(){
      ...
      //验证短信验证码
      $valid_status = check_ypsms_code($mobile, 2, $randcode);
      if(!($valid_status)){
          $this->error('短信验证码错误！');
      }
      //  给管理员发送邮件
      $this->sendMail($mobile);
      $user_api = new UserApi;
      $userInfo = $user_api->infoByMobile($mobile);
      //自动创建用户并登录
      if($userInfo == -1){
          $model = new UserModel();
          $from = 'home'; //来自官网
          $pwd = $model->autoCreateUser($mobile,$from);
          if($pwd){
              // 发送密码短信
              $tpl_id = 1699696;
              send_yptpl_sms($mobile,$tpl_id,array('pwd'=>$pwd));
          }
      }else{
          //登录用户
          D('Member')->login($userInfo['id']);
      }
      
      $this->success('开启极铺商城体验之路！',U('Index/index'));
   ...
   * 发送邮件
   */
  public function sendMail($mobile){
      $to = 'jinjianwei@jipukeji.com';
      //$to = '978989861@qq.com';
      $subject = '官网免费体验用户';
      $content = '电话联系方式：'.$mobile;
