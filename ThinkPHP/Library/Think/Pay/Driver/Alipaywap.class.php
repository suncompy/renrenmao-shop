...
class Alipaywap extends \Think\Pay\Pay {
  ...
  public function verifyNotify($notify){
     ...
     * @return 验证结果
     */
    public function verifyAppNotify($notify){
        if(empty($notify)) {//判断POST来的数组是否为空
            return false;
        } else {
            //生成签名结果
            //$isSign = true;
            $isSign = $this->getAppSignVeryfy($notify, $notify["sign"]);
            //获取支付宝远程服务器ATN结果（验证是否是支付宝发来的消息）
            $responseTxt = 'false';
            ...
            if (preg_match("/true$/i",$responseTxt) && $isSign) {
                $info['status'] = (($notify['trade_status'] == 'TRADE_SUCCESS') || ($notify['trade_status'] == 'TRADE_FINISHED')) ? true : false;
    ...
    /**
     * 获取返回时的签名验证结果  [app支付  rsa签名]
     * @param $para_temp 通知返回来的参数数组
     * @param $sign 返回的签名结果
     * @return 签名验证结果
     */
    function getAppSignVeryfy($para_temp, $sign) {
        //除去待签名参数数组中的空值和签名参数
        ...
        $isSgin = $this->rsaVerify($prestr, trim($this->config['public_key_path']), $sign);
        return $isSgin;
    }
    /**
     * RSA验签
    ...
    public function rsaVerify($data, $ali_public_key_path, $sign)  {
        if(file_exists($ali_public_key_path)){
