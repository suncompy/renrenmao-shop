...
class OrderEvent{
  ...
  public function getOrderNum($uid = 0){
    ...
    foreach($order_list as $l){
      if($l['o_status'] == 202 || $l['o_status'] == 205){ //已完成
        $return_data['success'] ++;
      }elseif($l['o_status'] == 0){
        $return_data['payment'] ++;
   ...
   */
  function doCount($item_ids, $cart_ids,$buynow = false){
    $result = array(
      'total_num' => 0,
      'total_quantity' => 0,
      'total_price' => 0.00,
    ...
    $cart_items = D('Order')->getCartItems($item_ids, $cart_ids, $buynow);
    ...
    foreach($result['supplier'] as $supp_id => &$supp){
      ...
      if( NoFreeAmount($supp_id) || (!NoFreeAmount($supp_id) && $free_amount > 0 && $supp['total_price'] < $free_amount) ){
  ...
  function getOrderTotalAmount($data){
    ...
    //计算使用满减、优惠券、礼品卡、积分、账户余额后的剩余订单金额
    $mj_before = $total_amount = sprintf('%.2f', $data['total_price'] + $data['delivery_fee'] - $data['manjian'] - $data['coupon_amount'] - $data['card_amount'] - $data['score_amount']);
    //计算打折商品总价（不包含邮费） 
    if($total_amount > 0){
   ...
   */
  private function _countManjian(&$data){
    $total_price= $this->_setOrderManjianPrice($data['total_price']);
    $data['manjian'] = $data['total_price'] - $total_price;
  }
  /**
   * 使用优惠券
   * @author Justin <justin@jipu.com>
  ...
  private function _countCoupon(&$data){
    ...
    if($coupon_id){
      ...
      if($coupon){
        ...
        $sub_amount = sprintf('%.2f', $data['total_price'] + $data['delivery_fee'] - $data['manjian']);
  ...
  private function _countCard(&$data){
    ...
    if($card_id){
      ...
      if($card){
        ...
        $sub_amount = sprintf('%.2f', $data['total_price'] + $data['delivery_fee'] - $data['manjian'] - $data['coupon_amount']);
  ...
  private function _countScore(&$data){
    ...
    if ($data['total_price'] > 0 ) {
      ...
      //减去优惠券后的订单金额$data['score_amount'] - $data['finance_amount'] -
      $sub_amount = sprintf('%.2f', $data['total_price'] + $data['delivery_fee'] - $data['manjian'] - $data['coupon_amount'] - $data['card_amount']);
  ...
  private function _countFinance(&$data){
    ...
    if(I('post.is_use_finance') == 1){
      if($member['finance'] > 0 && $data['total_price'] > 0){
        //计算使用其他支付方式后剩余的订单金额
        $sub_amount = sprintf('%.2f', $data['total_price'] + $data['delivery_fee'] - $data['manjian'] - $data['coupon_amount'] - $data['score_amount'] - $data['card_amount']);
  ...
  private function _filterOrderData(&$data){
    ...
    //满减
    if($data['manjian'] > 0){
      $data['manjian_amount'] = $payment_data['manjian'] = $data['manjian']  ;
      //unset($data['manjian']);
    }
  ...
  function splitOrderBySupplier($order_id = 0){
    if($order_id){
      ...
      //邮费
      $payment_data = M('Payment')->field('delivery_data, coupon_amount , score_amount,card_amount,manjian')->find($order_data['payment_id']);
      $delivery_data = unserialize($payment_data['delivery_data']);
      //降序排列，更精确的比例匹配
      $suppliers = M('OrderItem')->where('order_id ='.$order_id)->field('sum(sub_total) as amount, supplier_id')->group('supplier_id')->order('amount asc')->select();
      $supplier_data = array_column($suppliers , 'supplier_id');
      $counts = $count = count($supplier_data);
      $k = 0;
      $memo = unserialize($order_data['memo']);
      //第三方支付总额
      $all_total_amount = $order_data['total_amount'];
      // 账户余额使用金额
      $all_finance_amount = $order_data['finance_amount'];
      //total_price 商品总价格
      $all_total_price = $order_data['total_price'];
      //积分支付
      $all_score_amount  = $payment_data['score_amount'];
      //礼品卡支付部分
      $all_card_amount   = $payment_data['card_amount'];
      //优惠卷部分
      $all_coupon_amount = $payment_data['coupon_amount'];
      //满减部分
      $all_manjian_amount = $payment_data['manjian'];
      //拆分订单
      do{
        ...
        //重置新订单数据
          ...
          $order_item = M('OrderItem')->where($where)->select();
          $new_order_item_item_ids = array_column($order_item, 'item_id');
          $new_order_item_ids = array_column($order_item, 'id');
          ...
          foreach ($order_item as $v) {
              $new_order_data['total_price'] += $v['sub_total'];
          ...
          //比例
          $rate = bcdiv( sprintf("%.2f", $new_order_data['total_price'] + $new_order_data['delivery_fee'] ),  sprintf("%.2f",$order_data['total_price'] + $order_data['delivery_fee']) , 2 );
          ...
          if ($order_data['total_amount'] != ($order_data['total_price'] + $order_data['delivery_fee'])) {
            //有优惠
            $new_order_data['total_amount'] = doCountPrice($rate,  $all_total_amount , $k+1 , $counts , $order_data['total_amount'] , $new_order_data['total_price'] + $new_order_data['delivery_fee']);
          } else {
            //无优惠
            $new_order_data['total_amount'] = $new_order_data['total_price'] + $new_order_data['delivery_fee'];
          }
          $all_total_amount = sprintf("%.2f", $all_total_amount - $new_order_data['total_amount']);
          // 剩余待支付额度，最低支付
          $temp_amount = $new_order_data['total_price'] + $new_order_data['delivery_fee'] - $new_order_data['total_amount'];
          //余额
          if($order_data['finance_amount'] > 0 && $temp_amount > 0){
            $new_order_data['finance_amount'] = doCountPrice($rate,  $all_finance_amount , $k+1 , $counts ,$order_data['finance_amount'] , $temp_amount);
            $all_finance_amount = sprintf('%.2f' , $all_finance_amount - $new_order_data['finance_amount']);
            $temp_amount = sprintf('%.2f' , $temp_amount - $new_order_data['finance_amount']);
          } 
          if($payment_data['score_amount'] > 0 && $new_order_data['total_price'] + $new_order_data['delivery_fee'] - $new_order_data['finance_amount'] > 0 && $temp_amount > 0){
            $new_order_data['score_amount'] = doCountPrice($rate,  $all_score_amount , $k+1 , $counts ,$payment_data['score_amount'], $temp_amount);
            $all_score_amount = sprintf('%.2f' , $all_score_amount - $new_order_data['score_amount']);
            $temp_amount = sprintf('%.2f' , $temp_amount - $new_order_data['score_amount']);
          }
          //优惠卷部分
          if($payment_data['coupon_amount'] > 0 && $new_order_data['total_price'] + $new_order_data['delivery_fee'] - $new_order_data['finance_amount'] - $new_order_data['score_amount']> 0 && $temp_amount > 0){
            $new_order_data['coupon_amount'] = doCountPrice($rate,  $all_coupon_amount , $k+1 , $counts ,$payment_data['coupon_amount'], $temp_amount);
            $all_coupon_amount = sprintf('%.2f' , $all_coupon_amount - $new_order_data['coupon_amount']);
            $temp_amount = sprintf('%.2f' , $temp_amount - $new_order_data['coupon_amount']);
          }
          //礼品卡支付部分
          if($payment_data['card_amount'] > 0 && $new_order_data['total_price'] + $new_order_data['delivery_fee'] - $new_order_data['finance_amount'] - $new_order_data['score_amount'] - $new_order_data['coupon_amount']> 0 && $temp_amount > 0){
            $new_order_data['card_amount'] = doCountPrice($rate,  $all_card_amount , $k+1 , $counts ,$payment_data['card_amount'], $temp_amount);
            $all_card_amount = sprintf('%.2f' , $all_card_amount - $new_order_data['card_amount']);
            $temp_amount = sprintf('%.2f' , $temp_amount - $new_order_data['card_amount']);
          }
          //满减支付部分
          if($payment_data['manjian'] > 0 && $new_order_data['total_price'] + $new_order_data['delivery_fee'] - $new_order_data['finance_amount'] - $new_order_data['score_amount'] - $new_order_data['coupon_amount'] -  $new_order_data['card_amount']> 0 && $temp_amount > 0){
            $new_order_data['manjian_amount'] = doCountPrice($rate,  $all_manjian_amount , $k+1 , $counts ,$payment_data['manjian'], $temp_amount);
            $all_manjian_amount = sprintf('%.2f' , $all_manjian_amount - $new_order_data['manjian_amount']);
            $temp_amount = sprintf('%.2f' , $temp_amount - $new_order_data['manjian_amount']);
          }
          if($temp_amount > 0 && $all_total_amount > 0){
            $atemp = min($all_total_amount, $temp_amount) ; 
            $all_total_amount -= $atemp;
            $temp_amount -= $atemp;
            $new_order_data['total_amount'] += $atemp;
          }
          if($temp_amount > 0 && $all_finance_amount > 0){
            $ftemp = min($all_finance_amount, $temp_amount) ; 
            $all_finance_amount -= $ftemp;
            $temp_amount -= $ftemp;
            $new_order_data['finance_amount'] += $ftemp;
          }
          if($temp_amount > 0 && $all_score_amount > 0){
            $stemp = min($all_score_amount, $temp_amount);
            $all_score_amount -= $stemp;
            $temp_amount -= $stemp;
            $new_order_data['score_amount'] += $stemp;
          }
          if($temp_amount > 0 && $all_coupon_amount > 0){
            $ctemp = min($all_coupon_amount, $temp_amount);
            $all_coupon_amount -= $ctemp ;
            $temp_amount -= $ctemp ;
            $new_order_data['coupon_amount'] += $ctemp ;
          }
          if($temp_amount > 0 && $all_card_amount > 0){
            $catemp = min($all_card_amount, $temp_amount);
            $all_card_amount -= $catemp;
            $temp_amount -= $catemp;
            $new_order_data['card_amount'] += $catemp;
          }
          if($temp_amount > 0 && $all_manjian_amount > 0){
            $mtemp = min($all_manjian_amount, $temp_amount);
            $all_manjian_amount -= $mtemp;
            $temp_amount -= $mtemp;
            $new_order_data['manjian_amount'] += $mtemp;
          }
        } 
        //对应留言
        ...
        if($counts > 1){
          $new_order_id = M('Order')->add($new_order_data);
           ...
           $new_order_id = M('Order')->save($new_order_data);
        ...
        if($new_order_id) {
          ...
          D('SdpRecord')->sdpOrder($new_order_id);
          //推广联盟订单返钱(桌面商家)
          if( C('DIS_ORDERSTATUS') == 1){
              distribute($new_order_id ,$new_order_data['uid']);
          }
          //商品赠送积分
          $new_order = M('Order')->getById($new_order_id);
          $new_order['order_id'] = $new_order_id;
          A('Pay', 'Event')->doScoreGive($new_order,2);
        }
        //更新原订单
      ...
      //暂不删除原订单 只是修改其状态 -2只做记录使用
      if($counts > 1) M('Order')->where('id=' . $order_id)->setField('status',-2);
    }
  }
   ...
   * @author Justin <justin@jipu.com>
   */
  public function getManjianRule($total_amount){
    if(!$total_amount || $total_amount < 0.01) return false;
    $now = time();
    $where = array(
      'start_time' => array('elt', NOW_TIME),
      'expire_time' => array('gt', NOW_TIME-86400),
      'status' => 1,
      'man' => array('elt' , $total_amount),
    );
