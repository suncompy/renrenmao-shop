...
class ItemPropertyController extends AdminController {
  ...
  public function index(){
    ...
    if($keywords){
        ...
        $category = M('ItemCategory')->where($map)->field('id')->select();
        $cid = array_column($category,'id');
      //  $maps['cname'] =  array('like', '%'.$keywords.'%');
      //  $property = M('ItemProperty')->where($maps)->field('cid')->select();
      //  $cids = array_column($property,'cid');
      //  $p_cids = array_merge($cid,$cids);
        if(!$cid)$cid=array(0);
    }
     if($cid)$where['cid'] = array('in',$cid);
    //实例化商品属性模型
    ...
    //查询条件初始化
    $where['user_id'] = UID;
    $data = M('ItemProperty')->where($where)->select();
    foreach($data as $K => $v){
        $status = M('ItemCategory')->where(array('id'=>$v['cid']))->getField('status');
        $status == 0 ? M('ItemProperty')->where(array('id'=>$v['id']))->delete() : null;
    }
  ...
  public function getPictureConfigData($item_id = null, $prp_id = null, $code = null){
    ...
    if($item_extend){
      ...
      if($item_extend){
        //颜色配置说明
        $colorEname = $this->_getProFieldName($item_id);
        $color_array = $item_extend['property'][$colorEname];
   ...
   */
  private function _getProFieldName( $item_id,$type='specification',$formtype='color',$return='ename' ){
    $cat_ids = M('Item')->where('id='.$item_id)->field('cid_1,cid_2,cid_3')->find();
    $cate_ids = array_values($cat_ids);

    $map['type'] = $type;
    $map['cid'] = array('IN',implode(',',$cate_ids));
    $map['formtype'] = $formtype;

    $res = M('ItemProperty')->where($map)->getField($return);
