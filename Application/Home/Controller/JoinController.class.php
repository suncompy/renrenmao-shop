...
class JoinController extends HomeController{
    ...
    public function _initialize(){
        ...
        $this->member = get_member_info(UID);
    }
    ...
    public function detail(){
        ...
        //获取商品数据
        $data = D('Join')->detail($id ,true ,$activity_id);
        // var_dump($data);die;
        if(empty($data)){
        ...
        $where = 'a.item_id ='.$data['id'].' and j.status =1  and a.status=0 and a.etime >'.time();//.' and etimte >'.time()
        $this->joinlist = M('Join_list')->alias('a')->join('LEFT JOIN __JOIN__ j on j.id=a.activity_id')->join(' LEFT JOIN __JOIN_ORDER__ b on a.id=b.join_id and a.reg_uid=b.uid')->join(' LEFT JOIN __ORDER_SHIP__ c on b.payment_id=c.payment_id')->where($where)->field('a.*,b.id as joid,c.ship_area')->group('a.id')->select();
        $this->data = $data;
    ...
    public function addCookie(){
        ...
        //限购数量
        $quota_num = get_quota_joinnum($item_id);
    ...
    public function join(){
        ...
        $where['a.etime'] = array('gt' , time());
        $where['mm.status'] = 1;
        $result = M('join_list')->alias('a')->join('LEFT JOIN __JOIN_ITEM__ b on a.item_id=b.item_id' )->join(' LEFT JOIN __JOIN_ORDER__ c on a.reg_uid=c.uid and a.id=c.join_id ')->join(' LEFT JOIN __ITEM__ d on d.id=a.item_id')->join('LEFT JOIN __JOIN__ mm on mm.id=a.activity_id and mm.id=b.join_id')->where($where)->join(' LEFT JOIN __ORDER_SHIP__ e on e.payment_id=c.payment_id')->field('a.*,b.*,c.*,d.thumb,d.price,d.id as gid,a.stime as hdstime,e.ship_area , a.etime as hdetime,a.id as join_id,b.price as hdprice,a.status as hdstatus,c.id as myhdid')->find();
        if(!$result){
            $this->error('活动已结束');
        }
        ...
        $result['spec'] = kaspec($result['spec']) ;
    ...
    function joinlist( $goods_id ){
        ...
        $where = array(
            ...
            'j.status'    => 1,
            'j.etime'     => array('gt' , time()),
        );
        $data = M('Join_item')->alias('a')->join('RIGHT JOIN __JOIN__ j on j.id=a.join_id')->join(' __ITEM__ b on a.item_id=b.id ' )->where($where)->field('a.*,b.thumb,b.name as goods_name')->select();
        foreach($data as $k => $v){
            $temp = get_cover($v['thumb']);
            $data[$k]['thumb'] = $temp['path'];
        }
        return $data;
    }
    ...
    public function joinOrder(){
        ...
        $map['i.status'] = 1;
        $fields = 'j.* ,j.status as jstatus, o.join_sn,o.paytime,o.total_amount,o.status,i.name,i.summary,i.thumb,s.ship_area,s.ship_address,o.ctime as otime ,o.item_id,o.id,j.id as jid ,o.ltime , o.ltime as hdetime';
        $limit = 10;
        $order = 'o.id desc';
        $prefix  = C('DB_PREFIX');
        ...
        foreach($lists as $k => $v){
            if($v['jstatus'] == 1 && $v['status'] == 0){
                $lists[$k]['out'] = 1 ;
                continue;
            }
            if($v['status'] == '-1' && time() > min($v['hdetime'] ,gethdtime($v['item_id']) )  ){
                $lists[$k]['out'] = 1;
            }elseif($v['jstatus'] == 0 && $v['paytime'] > 0 &&  min($v['hdetime'] ,gethdtime($v['item_id']) ) > time()  ){
        ...
        IS_AJAX ? $this->display('joinList') : $this->display('joinOrder');
    } 
    ...
    function joininfo(){
        if(IS_AJAX){
            ...
            if($id > 0){
                ...
                $where['a.etime']  = array('gt' , time());
                $where['j.status'] = 1;
                $result = M('join_list')->alias('a')->join(' LEFT JOIN __JOIN_ORDER__ c on a.reg_uid=c.uid and a.id=c.join_id ')->join('LEFT JOIN __JOIN__ j on a.activity_id=j.id')->where($where)->field('c.id ,c.item_id,c.join_id ,c.spec,c.item_code')->find();
                if($result){
                    $result['price'] = getjoinprice($result['id']);
                    $result['spec']  = kaspec($result['spec']);
                    $result['status']  =true;
        ...
        echo json_encode( array( 'status' => false ,'info' => '参团失败，换个试试') ) ;
        exit;
    }
}
