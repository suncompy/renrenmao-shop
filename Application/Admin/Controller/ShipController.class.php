...
class ShipController extends AdminController{
  ...
  public function add($order_id = 0){
    ...
    //获取配送信息
    $delivery = unserialize(M('Payment')->where('id='.$order['payment_id'])->getField('delivery_data'));
    $order['delivery_id'] = $delivery[$order['supplier_ids']]['delivery_id'];
    //物流模板信息
        ...
        if(IS_POST){
           $order_id = I('post.order_id');
            ...
            if(false !== $model->update()){
               //发货成功，短信通知用户
                $tpl = 'SENDORDER_NOTIFY';
                if($tpl){
                    send_yptpl_sms($mobile,$tpl,array('key'=>''));
                }
