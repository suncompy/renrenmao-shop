...
class Common_util_pub{
  ...
  function __construct($trade_type){
    $this->conf = new WxPayConf_pub($trade_type);
  }
  ...
  public function getSign($Obj){
    ...
    //签名步骤二：在string后加入KEY
    $String = $String."&key=".$this->conf->KEY;
    //dump($String);
    //签名步骤三：MD5加密
    $String = md5($String);
  ...
  function postXmlSSLCurl($xml, $url, $second = 30){
    ...
    curl_setopt($ch, CURLOPT_SSLCERT, $this->conf->SSLCERT_PATH);
    //默认格式为PEM，可以注释
    curl_setopt($ch, CURLOPT_SSLKEYTYPE, 'PEM');
    curl_setopt($ch, CURLOPT_SSLKEY, $this->conf->SSLKEY_PATH);
    //post提交方式
    curl_setopt($ch, CURLOPT_POST, true);
    curl_setopt($ch, CURLOPT_POSTFIELDS, $xml);
    $data = curl_exec($ch);
...
//     $loginfo['xml'] = $xml;
//     $loginfo['key']  = $this->conf->SSLKEY_PATH;
//     $loginfo['cert'] = $this->conf->SSLCERT_PATH;
//     $loginfo['url'] = $url;
//     $loginfo['return'] = $data;
//     $loginfo['curl_info'] = $curl_info;
//     $loginfo['curl_errno'] = $curl_errno;
//     $loginfo['lasttime'] = $lasttime;
//     set_log('ssl_refund',$loginfo,'证书');
    
    //返回结果
    if($data){
      return $data;
    }else{
      echo "curl出错，错误码:$curl_errno"."<br>";
      echo "<a href='http://curl.haxx.se/libcurl/c/libcurl-errors.html'>错误原因查询</a></br>";
      return false;
    }
  }
...
class Wxpay_client_pub extends Common_util_pub{
  ...
  function createXml(){
    ...
    $this->parameters["appid"] = $this->conf->APPID; //公众账号ID
    $this->parameters["mch_id"] = $this->conf->MCHID; //商户号
...
class UnifiedOrder_pub extends Wxpay_client_pub{
  ...
  function getPrepayId(){
    ...
    if($this->result["prepay_id"]){
      F('WeixinPay/prepay_id/'.date('Ym').'/'.date('d').'/'.date('His').'-'.($this->result["prepay_id"]), $this->result);
    }else{
      F('WeixinPay/prepay_id/error/'.date('Ym').'/'.date('d').'/'.date('His').'-'.mt_rand(0, 99999), $this->result);
    }
...
class NativeCall_pub extends Wxpay_server_pub{
  ...
  function createXml(){
    if($this->returnParameters["return_code"] == "SUCCESS"){
      $this->returnParameters["appid"] = $this->conf->APPID; //公众账号ID
      $this->returnParameters["mch_id"] = $this->conf->MCHID; //商户号
...
class JsApi_pub extends Common_util_pub{
  ...
  function createOauthUrlForOpenid(){
    $urlObj["appid"] = $this->conf->APPID;
    $urlObj["secret"] = $this->conf->APPSECRET;
  ...
  public function getParameters(){
    $jsApiObj["appId"] = $this->conf->APPID;
    $timeStamp = time();
   ...
   * 	作用：设置jsapi的参数
   */
  public function getParametersArr($prepay_id){
    $jsApiObj["appId"] = $this->conf->APPID;
    $timeStamp = time();
    $jsApiObj["timeStamp"] = "$timeStamp";
    $jsApiObj["nonceStr"] = $this->createNoncestr();
