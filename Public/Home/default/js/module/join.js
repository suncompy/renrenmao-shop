/**
 *拼团模块：商品规格选择，加入收藏，更改数量
 */
define('module/join', function(require, exports, module){
  ...
  var join = { 
    ...
    sku_selected_size: 0,
    spc_data_info: '',
    init: function(spc_data_info){
      var _self = this,
          fixed_navbar = $('#J_fixed_navbar');
  
      //单独购买
      $('.single-buy').click(function(){
          var item_id = $('#J_item_id').val();
          window.location.href = $.U('/Item/detail','id='+item_id);
      });
       $('.J_join_buy1.psd').on('click', function(){
         _self.setjoinvalue(this) ;
          
      });
      //  $('.J_join_buy1').on('click', function(){
          ...
          if(_self.spc_data_info){
            ...
            if(!$.isEmptyObject(spec_data_info)){
              $.each(spec_data_info, function(k, spec){
                ...
                spec_data[spec.spc_code]['price'] = spec.first_price;
    ...
    // 设置参团属性
    setjoinvalue:function(obj){
      var zhi = $(obj).attr('val');
      var url = $.U('Join/joininfo');
      $.ajax({
          type: 'POST',
          url: url,
          data: {id :zhi},
          ...
          success: function (res){
            if(res.status == true){
              $('#J_item_id1').val(res.item_id);
              $('#J_item_joinid1').val(res.join_id);
              $('#J_item_spec1').val(res.spec);
              $('#J_item_code1').val(res.item_code);
              $('#J_item_price1').val(res.price);
              add1();
            }else{
              $.ui.error(res.info);
              return false;
    ...
    add: function (){
      var _self = this,
              ...
              url = $.U('Join/addCookie'),
              join_id = $('#J_item_joinid').val() ,
              data = {item_code: item_code , item_id: item_id, quantity: quantity, price: price ,buynow : 1 , jorderid:join_id};
      ...
      $.ajax({
        ...
        success: function (res){
          if(res.status == 1){
              window.location.href = $.U('Join/order');
    ...
    pay: function (order_id) {
      $('#J_payment_' + order_id).on('submit', function () {
        ...
        if (paymeny_type === 'wechatpay') {
          ...
          $.ui.load($.U('/Join/weixinPayCode', 'order_id=' + order_id), '请用微信扫码支付');
          return false;
        } else {
          var pay_redirect = function () {
            window.location.href = $.U('Join/joinOrder');
          }
    ...
    setStatus: function (join_id, type) {
      ...
      if(seckill){
        ...
        if(now > seckill.expire_time){
          ...
          if(now < seckill.start_time){
            //尚未开始
            $('.J_seckill_time').html('距团购开始<span class="text-danger day">00</span>天<span class="text-danger hour">00</span>小时<span class="text-danger minute">00</span>分钟<span class="text-danger second">00</span>秒');
            this.countDown(this.unix_to_datetime(seckill.start_time), ".J_seckill_time");
          }else{
            //进行中
            $('.J_seckill_time').html('距团购结束<span class="text-danger day">00</span>天<span class="text-danger hour">00</span>小时<span class="text-danger minute">00</span>分钟<span class="text-danger second">00</span>秒');
    ...
    /*倒计时*/
    showtime : function(obj ,id){
      var time = $(obj).find(id).attr('val');
      var bla = this.unix_to_datetime(time);
      this.countDown(  bla ,id);
    },
    /*倒计时*/
 ...
 // 参团
    function add1 (){
      var _self = this,
        item_code = $('#J_item_code1').val(),
        item_id = $('#J_item_id1').val(),
        quantity = $('#J_item_quantity1').val(),
        price = $('#J_item_price1').val(),
        spec = $('#J_item_spec1').val(),
        join_id = $('#J_item_joinid1').val() ,
        url = $.U('Join/addCookie'),
        data = {item_code: item_code , item_id: item_id, quantity: quantity, price: price ,buynow : 1 , jorderid:join_id};
