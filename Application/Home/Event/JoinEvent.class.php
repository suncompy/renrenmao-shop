...
class JoinEvent {
    ...
    function doCount(){
        ...
        foreach($cart_items as $item){
            ...
            //商品价格 
            $price = D('Join')->getItemPrice($item['item_id'], $item['item_code'] ,$item['jorderid']);
            //运费
    ...
    public function checkItemStock($item_id,$item_code,$num){
        ...
        if($item_id && $num){
            ...
            //判断是否配置了规格分库存
            $quantity = $item_spc_model->alias('a')->join('LEFT JOIN __JOIN__ b on b.id=a.activity_id')->where(' a.item_id='. $item_id.' and b.status=1')->getFieldBySpcCode($item_code, 'quantity');
            if($quantity){
                if($quantity < $num){
                    return false;
                }
            }else{
                $quantity = $item_model->alias('a')->join('LEFT JOIN __JOIN__ b on b.id=a.join_id')->where('b.status=1')->getFieldByItemId($item_id, 'stock');
                if($quantity < $num){
                    return false;
                }
            }
    ...
    public function checkjoin($join_id){
        $join = M('Join_list')->alias('a')->join(' LEFT JOIN __JOIN_ITEM__ b on a.item_id=b.item_id and a.activity_id=b.join_id')->where('a.id ='.$join_id.' and a.status =0')->field('a.*,b.join_num')->find();
        if(!empty($join)){
    ...
    public function createorder($join_id){
        $info = M('join_order')->alias('a')->join('__PAYMENT__ b on a.payment_id=b.id')->field('a.* , b.payment_type , b.create_time as paytime')->where('a.status =1  and a.join_id = '.$join_id)->select();
        if(!empty($info[0]['item_id'])){
            ...
            foreach($info as $k => $v){
                ...
                $data = array(
                    ...
                    'delivery_fee'  => $v['delivery_fee'] ? $v['delivery_fee']: 0 ,
                    'memo'          => $v['memo'],
                    'order_from'    => 1,
                    'create_time'   => time(),
                    'update_time'   => time(),
    ...
    public function updteStockAndBuy($order_id){
        ...
        if($orderData){
            ...
            $ac_id = getactivityid($orderData['item_id']);
            //更新活动库存
            M('join_item_spec')->where(array('spc_code' => $orderData['item_code'] ,'activity_id'=> $ac_id))->setDec('quantity',$orderData['total_quantity']);
            //更新活动总库存
            $join_model->where(array('item_id' => $orderData['item_id'] , 'join_id'=> $ac_id))->setDec('stock',$orderData['total_quantity']);
