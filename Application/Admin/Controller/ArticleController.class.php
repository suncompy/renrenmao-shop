...
class ArticleController extends AdminController{
  ...
  public function system(){
    $lists = $this->lists('ArticleSystem',array(),'id asc');
    $this->assign('lists',$lists);
    $this->meta_title = '系统文章';
    $this->display();
  }
  ...
  public function editSystem(){
    $id = I('request.id');
      ...
      if(IS_POST && IS_AJAX){
          ...
          $data['code'] = I('code');
          $data['ctime'] = time();
          empty($data['title']) && $this->error('请填写文章标题');
          if(!empty($id)){
              $type = '编辑';
          ...
          $rst !== false ? $this->success($type.'成功！',U('system')) : $this->error($type.'失败！');
      }
      $data = array();
      $meta_title = '添加系统文章';
