...
class HomeController extends BaseController{
  ...
  protected function _initialize(){
    ...
    //   $this->assign('keywords', arr2str(C('SEARCH_KEYWORDS')));
    // }
  ...
  private function getUsers(){
    if(is_login()){
      if (C('TS_LOGIN')) {
        $tid = D('TsUser')->where(array('uid'=>UID))->getField('tid');
        define('TID', $tid);
      }
    ...
    //站内消息未读条数
    $this->message_unread = A('Message', 'Event')->getUnreadNum(UID);
  }
