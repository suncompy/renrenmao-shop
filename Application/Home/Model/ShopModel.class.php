...
class ShopModel extends Model{
  ...
  protected $_auto = array(
    ...
    //array('pc_pic','_get_shopbanner', self::MODEL_BOTH, 'callback'),
  ...
  public function update($data){
    ...
    if (is_mobile()) {
      $data['wap_pic'] = $this->_get_shopbanner();
    }else{
      $data['pc_pic'] = $this->_get_shopbanner();
    }
    if(!$data){
      return '';
    }
    ...
    $data['shopper_code'] = is_numeric($data['shopper_code']) ? get_cover(I('post.shopper_code'), 'path'):$data['shopper_code'];
  ...
  public function _getAuditData(){
    ...
    }elseif(mb_strlen($data['intro']) < 10 || mb_strlen($data['intro']) > 100){
      $this->error = '请输入理由（字数大于10小于100）';
    }
    if($this->error){
      return false;
    }else{
  ...
  public function removeItem($itemid){
     ...
     * @author Max.Yu <max@jipu.com>
     */
    public function addBanner($banners = 0){
        $shop = $this->getByUid(UID);
        $new_ids = str_remove(0, str_add($banners, $shop['banner_ids']));
        return $this->where(array('id' => $shop['id']))->setField('banner_ids', $new_ids);
    }
    ...
    public function removeBanner($bannerid){
        $shop = $this->getByUid(UID);
        $new_ids = str_remove($bannerid.',0', $shop['banner_ids']);
        return $this->where(array('id' => $shop['id']))->setField('banner_ids', $new_ids);
    }
   ...
   */
  protected function _get_shopbanner(){
    //保存图片到服务器
    $wap_pic_id = I('post.wap_pic_id', '');
    $pc_pic_id = I('post.pc_pic_id', 0);
    //PC
    if($pc_pic_id){
      return is_numeric($pc_pic_id) ? get_cover($pc_pic_id, 'path') : $pc_pic_id;
    }
    if($wap_pic_id){
      return is_numeric($wap_pic_id) ? get_cover($wap_pic_id, 'path') : $wap_pic_id;
    }
    if (is_mobile()) {
      return $this->getFieldById(I('post.id'), 'wap_pic');
    }else{
      return $this->getFieldById(I('post.id'), 'pc_pic');
    }
