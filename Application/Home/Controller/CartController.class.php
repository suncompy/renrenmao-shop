...
class CartController extends HomeController{
  ...
  public function index(){
    ...
    if(!is_login()){
      ...
      if($list){
        ...
        foreach($list as $key => &$value){
          ...
          $quantity = ($value['quantity'] > $value['stock']) ? $value['stock'] : $value['quantity'];
          $value['amount'] = $quantity * $value['price'];
          $value['subAmount'] = sprintf("%.2f", $quantity * $value['price']);
          $value['subQuantity'] = $quantity;
        }
      }
    ...
    //重定义
    foreach ($list as $key => &$value) {
        $subAmount += $value['subAmount'];
        $subQuantity += $value['subQuantity'];
    }
    $cart_count['total_amount'] = $subAmount;
    if($subQuantity > 0 )$cart_count['total_quantity'] = $subQuantity;
    ...
    if($buynow == 1){
      ...
      if(!is_login()){
        ...
        if(($exists_num + $quantity) > $quota_num){
          ...
          if($current_items){
            //判断加入购物车商品是否存在 规格参数 商品ID
            $current_item_ids = get_sub_by_key($current_items, 'item_id');
            $current_item_codes = get_sub_by_key($current_items, 'item_code');
            foreach ($current_item_ids as $key => &$value) {
              $arr[$key] = $value.'-'.$current_item_codes[$key];
            }
            $str = $item_id.'-'.$item_code;
            if (in_array($str,$arr)) {
              foreach($current_items as $key => &$value){
                if( ($value['item_id'] == $item_id) && ($value['item_code'] == $item_code) ){
