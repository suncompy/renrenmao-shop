 ...
 * OneThink常量定义
 */
const ONETHINK_VERSION = '5.0_20170606';
...
function is_login(){
  ...
  if(empty($user)){
    //Ts用户系统
    if(C('TS_LOGIN') && I('get.oauth_token') && I('get.oauth_token_secret') && I('get.uid')){
      $user_api = new Common\Api\UserApi();
      $uid = $user_api->getTsUser(I('get.oauth_token'), I('get.oauth_token_secret'), I('get.uid'));
      return $uid;
    } else {
      return 0;
    }
...
// | Author: Fourth Pu <540690666@qq.com|ppssone@live.cn>
// |-----------------------------------------------------------------------------------
// | 精确计算 $part 计算部分 , $all 总价部分 ,$left 剩余部分 , $temp 当前指针 , $count 总计数 , 
// |$maybe可能部分 价格需要自己计算 其余优惠可用比例计算出来
// .-----------------------------------------------------------------------------------
  function doCountPrice($rate,  $left , $temp , $count ,$total , $current){
    if($temp == $count)return $left;
    if($left < 0)return 0;
    $rate = $rate > 0.01 ?  $rate : 0.01;
    $last = bcmul($rate , $total , 2 ) > 0 ? bcmul($rate , $total , 2 ) : 0;
    return sprintf("%.2f", floatval($last) > floatval($current) ? $current : $last);
  } 
/**
 * 检测EDU系统用户是否登录
 * @return integer 0-未登录，大于0-当前登录用户ID
 ...
 */
function is_edu_login(){
  $user = session('user_auth');
  if (empty($user)) {
    //EDU用户系统
    if(C('EDU_LOGIN') && I('get.u') && I('get.e')){
      $user_api = new Common\Api\UserApi();
      $uid = $user_api->getEduUser(I('get.u'), I('get.e'));
      return $uid;
    } else {
      return 0;
    }
...
function msubstr($str, $start = 0, $length, $charset = "utf-8", $suffix = true){
  ...
  return mb_strlen($str ,'utf-8') > $length && $suffix ? $slice.'...' : $slice;
}
...
function get_mobile($uid){
  if(!$uid) return false;
  return M('User')->where('id ='.$uid)->getField('mobile');
}
/**
 * 根据用户ID获取用户昵称
 * @param integer $uid 用户ID
 * @return string       用户昵称
 */
function get_nickname($uid = 0,$type = false){
  static $list;
  if(!($uid && is_numeric($uid))){ //获取当前登录用户名
    return session('user_auth.nickname') ? session('user_auth.nickname') : get_hidden_mobile(session('user_auth.mobile'));
  }
...
function get_images_info($images_ids, $field = null){
  ...
  if(is_array(array_filter($images_ids))){
    $images_ids = rtrim(implode(',', $images_ids),',');
  }else{
    $images_ids = rtrim($images_ids,',');
  }
...
function get_image_thumb($image_src, $width, $height, $type = 3, $x = 0, $y = 0){
  ...
  if(is_file($image_path)){
    ...
    return $image_path;
  }elseif(strpos($image_src,"http") !== false){
      //图片可能来自于微信
      return $image_src;
  }else{
    //原图不存在
...
function rotate($image){
  ...
  if(function_exists('image_type_to_extension')&&is_int($image)){
  ...
  //根据Orientation判断图片方向
  if ($srcimg && $exif && isset($exif['Orientation']))
  {
      $ort = $exif['Orientation'];
      if ($ort == 6 || $ort == 5)
          $srcimg = imagerotate($srcimg, 270, null);
      if ($ort == 3 || $ort == 4)
          $srcimg = imagerotate($srcimg, 180, null);
      if ($ort == 8 || $ort == 7)
          $srcimg = imagerotate($srcimg, 90, null);
      if ($ort == 5 || $ort == 4 || $ort == 7)
          imageflip($srcimg, IMG_FLIP_HORIZONTAL);
      //翻转图片替换原图    
      $imagefun = 'image'.($type=='jpg' ? 'jpeg' : $type);
      $imagefun($srcimg, $image);                
  }
  //销毁图片资源
  imagedestroy($srcimg);
}
 ...
 * @author Max.Yu <max@jipu.com>
 */
function send_email_origin($to, $tpl, $subject = array(), $data = array(), $attachment = null){
  return false;
  // 获取邮件模板
  $map_tpl = array(
    'name' => $tpl,
...
function invite_code($code){
  ...
  if(is_numeric($code)){
    ...
    return base_convert($code, 10, 16).'s';
  }else{
    return base_convert(rtrim($code , 's'), 16, 10);
  }
}
// .-----------------------------------------------------------------------------------
// | Version: 2017
...
// | 推广码加密 解密
// .-----------------------------------------------------------------------------------
function union_code($code , $do = null){
  if(!$code) return false;
  $temp = substr( base64_encode($_SERVER['HTTP_HOST']) , 4 ,2);
  if(is_numeric($code) && !$do){
    return invite_code($code).$temp;
  }else{
    if(!$step = stripos($code , $temp))return false;
    $coding = invite_code(substr($code , 0 , $step ));
    return is_numeric($coding) ? $coding : false ;
  }
}
 ...
 * @return mixed|string
 */
function get_hysms_content($tpl_id = 0,$param = array()){
    $content = '';
    if($tpl_id){
        $cache_name = 'yunpian_sms_tpl_'.$tpl_id;
            $sms = new Org\SMSHuyi\Sms();
            $content = $sms->getContent($tpl_id);
            S($cache_name,$content);
...
function send_yptpl_sms($mobile = '', $tpl_id = 0, $param = array()){
  ...
  if($tpl_id <= 0){
    ...
    if(C('SMS_TYPE')==1){
        ...
        $content = get_hysms_content($tpl_id,$param);
    }
    if($content){
    if(C('SMS_YUNPIAN.SEND_TEST') == true){ //短信测试模式
      ...
      return array('code' => 300, 'msg' =>C('SMS_YUNPIAN.SEND_TEST'));
       ...
       if(C('SMS_TYPE') == 1){
           ...
           //互亿
           $sms = new Org\SMSHuyi\Sms();
           $res = $sms->send( array('mobile' => $mobile, 'text' => $content));
       }
    }
    if($res['code'] == 0){
...
function get_finance_type_name($finance_type = null){
  $finance_type_name = array(
    ...
    'invite_register' => '关注注册成功返现',
  );
  return $finance_type ? $finance_type_name[$finance_type] : $finance_type_name;
}

/**
 * [get_accountcost_type_name description]
 * @author buddha <[email address]>
 * @param  [type] $finance_type [description]
...
function checkUserType($username){
  ...
  if(preg_match('/^([a-zA-Z0-9_-])+@([a-zA-Z0-9_-])+(.[a-zA-Z0-9_-])+/', $username))return 1 ;
  if(preg_match('/1[34578]{1}\d{9}$/', $username))return 2 ;
  return 3;
}
...
function get_count_payment_price($payment_id = 0, $type = 'total_price'){
  ...
  $order_count = M('Order')->where('payment_id='.$payment_id)->count();
  if ($order_count > 1) {
    $line['price'] = $line['price']/2;
  }
...
function get_o_status_text($o_status=''){
  $o_status_text = array(
    ...
    205 => '<span class="text-success">交易完成</span>',
    ...
    304 => '<span class="text-danger">退款处理中</span>',
...
function getsupplier($sid){
  ...
  /**
 * 加密函数 for api_password.
 *
 * @author martinsun <syh@sunyonghong.com>
 * @datetime 2017-03-13T15:32:17+080
 *
 * @version  1.0
 *
 * @param string $txt 加密文本
 * @param string $key 加密密钥
 *
...
/**
 * 解密函数 for api_password.
 *
 * @author martinsun <syh@sunyonghong.com>
 * @datetime 2017-03-13T15:33:27+080
 *
 * @version  1.0
 *
 * @param string $txt 使用加密函数加密后的字符串
 ...
 * @param  [type] $union_code [description]
 * @return [type]             [description]
 */
function handle_union($uid ,$union_code){
  //|| !M('Distribution')->where('user_id ='.$uid)->getField('oneagents')
  if(C('DIS_START') != 1)return -1;
  if(!$code = union_code($union_code , 1)){
    return 2;
  }else{
    $uinfo = M('User')->where('id ='.$code)->getField('status');
    if(!$uinfo)return 6;
    if($uinfo != 1)return 7;
    $ustatus = M('Union')->where('uid ='.$code)->getField('status');
    if(!$ustatus)return 8;
    if($ustatus == '0')return 9;
    if($code == $uid)return -2;
    if($agent = M('Distribution')->where('user_id ='.$code)->find()){
      $create_time = M('User')->where('id='.$uid)->getField('reg_time');
      if($my = M('Distribution')->where('user_id ='.$uid)->find()){
      //判断推广码是否是自己的下限
        $where['oneagents|twoagents|threeagents'] = $uid;
        $uids = M('Distribution')->where($where)->getField('user_id' ,true);
        if($uids && in_array($code ,$uids))return 4;
        if($uids){
          //不能让自己的下级的下级成为自己的代理
          $where1['oneagents|twoagents|threeagents'] = array('in' ,$uids);
          $uidss = M('Distribution')->where($where1)->getField('user_id' ,true);
          if($uidss && in_array($code ,$uidss))return 5;
        }
        $data['oneagents'] = $agent['user_id'];
        $data['twoagents'] = $agent['oneagents'] ? $agent['oneagents'] : 0;
        $data['threeagents'] = $agent['twoagents'] ? $agent['twoagents'] : 0;
        M('Distribution')->where('id ='.$my['id'])->save($data);
        if($one = M('Distribution')->where('oneagents ='.$uid)->getField('id' ,true)){
            $ones = array(
              'twoagents'   => $agent['user_id'],
              'threeagents' => $agent['oneagents'] ? $agent['oneagents'] : 0,
            );
            $one_map['id'] = array('in' , $one);
            M('Distribution')->where($one_map)->save($ones);
        }
        if( $two = M('Distribution')->where('twoagents ='.$uid)->getField('id' ,true)){
            $twos = array(
              'threeagents' => $agent['user_id'],
            );
            $two_map['id'] = array('in' , $two);
            M('Distribution')->where($two_map)->save($twos); 
        }
      }
        return true;
    }else{
 ...
 * @return [type]       [description]
 */
function union_error($type){
  if(!is_numeric($type))return false;
  switch ($type) {
    case 2:
      $mess = '该推广错误，请输入正确的推广码';
      break;
    case 3:
      $mess = '该用户还没开启推广联盟功能';
      break;
    case 4:
      $mess = '不能让自己的下级代理用户成为自己的代理用户';
      break;
    case 5:
      $mess = '不能让自己的下级代理用户的下级代理成为自己的代理用户';
      break;
    case 6:
      $mess = '该推广用户不存在';
      break;
