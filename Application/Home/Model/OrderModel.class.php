...
class OrderModel extends RelationModel{
  ...
  public function detail($map, $field = true){
    ...
    //其他优惠
    $info['discount_amount'] = sprintf('%.2f', ($info['discount_amount']-$info['coupon_amount']-$info['score_amount']-$info['card_amount']-$info['manjian_amount']));
    //支付方式
   ...
   * @author Justin <justin@jipu.com>
   */
  function checkItemStock($item_ids,$cart_ids,$buynow = false){
    $cart_items = $this->getCartItems($item_ids, $cart_ids, $buynow);
    if($cart_items){
   ...
   * @author Max.Yu <max@jipu.com>
   */
  function checkItemQuota($item_ids, $cart_ids, $buynow = false){
    $cart_items = $this->getCartItems($item_ids, $cart_ids, $buynow);
    if($cart_items){
   ...
   * @author Justin <justin@jipu.com>
   */
  function getCartItems($item_ids,$cart_ids, $buynow){
    //获取当前用户购物车所有商品
    if($buynow == false){
      $map['uid'] = UID;
      //$map['item_id'] = array('IN', $item_ids);
      $map['id'] = array('IN', $cart_ids);//修正 buddha 金海 20170813
  ...
  public function update($data = null, $buynow = false){
    ...
    //根据购物车商品规格，重新获取商品价格
    $cart_ids = I('post.cart_ids');//buddha 金海 20170813
    $order_count = A('Order', 'Event')->doCount($data['item_ids'], $cart_ids , I('post.buynow') == 1,$seckill);
    ...
    //更新满减到支付表
    if ($data['manjian']) {
      M('Payment')->where('id='.$payment_id)->setField('manjian', $data['manjian']);
    }
    unset($data['manjian']);
    ...
    if($result){    
      ...
      if($items){
        //升序排列商品，让每个商品都能进行合理比例分配
        // $items = my_array_multisort($items , 'price');
        $supp_ids = array();
        //买送-获取赠品
        $_arr = array();
        //分销返现钱
        $sdp_cashback = 0;
        //优惠卷 礼品卡金额
        // $DiscountAmount = M('Payment')->where('id ='.$payment_id.' and status = 1')->field('coupon_amount,card_amount')->find();
        // $coupon_amount = $DiscountAmount['coupon_amount'] ;
        // $card_amount   = $DiscountAmount['card_amount'] ;
        //第二件折扣
        ...
        foreach($items as $key => &$value){
          ...
          if(in_array($value['item_id'], $second_pieces_item_ids) && ($value['quantity'] >= 2)){
            ...
            $value['sub_total'] = sprintf("%.2f", $value['price'] * $value['quantity']);
            $value['memo'] = '';
          }
          // $CurrentPrice  =sprintf("%.2f", $value['sub_total']/$value['quantity']);
          // //比例
          // $rate = bcdiv($CurrentPrice , $data['total_price']) ;

          $value['fugou_dis_price'] = A('Fugou', 'Event')->getDisPriceByUser($value['item_id'], UID);
          $supp_ids[] = $value['supplier_id'];
          // $value['dis_price'] = $value['sub_total'];
          //精确计算出每个商品的实际价格，便于后续订单拆单和退款
          // if($DiscountAmount['coupon_amount'] > 0 && $value['sub_total'] > 0){
          //   $do_coupon =  doCountPrice($rate , $coupon_amount , $i , count($items) , $DiscountAmount['coupon_amount'] ,$CurrentPrice);
          //   $coupon_amount = sprintf('%.2f' , $coupon_amount - $do_coupon) ;
          //   $value['dis_price'] = sprintf('%.2f' , $value['sub_total'] - $do_coupon) ;
          // }

          // if($DiscountAmount['card_amount'] > 0 && $value['sub_total'] > 0){
          //   $do_card =  doCountPrice($rate , $card_amount , $i , count($items) , $DiscountAmount['card_amount'] , $CurrentPrice);
          //   $card_amount = sprintf('%.2f' , $card_amount - $do_card);
          //   $value['dis_price'] = sprintf('%.2f' ,$value['sub_total'] - $do_card);
          // }
          //买送-获取赠品
   ...
   * @author Justin <justin@jipu.com>
   */
  private function _checkOrderData($type=1){
    $item_ids = I('post.item_ids');
    $buynow = I('post.buynow');
    $cart_ids = I('post.cart_ids');//新增 buddha 金海 20170813
    if($buynow){
    ...
    $map = array(
        'status' => 0,
        'id'     => array('IN',$item_ids)
      );
    $count = M('Item')->where($map)->count();
    if ($count > 0) {
      $this->error = '部分商品已下架导致下单失败！';
      return false;
    }
    //检测库存 新增cart_ids buddha 金海 20170813
    if ($type == 1) {
      if(!$this->checkItemStock($item_ids, $cart_ids, $buynow == 1)){
        $this->error = '部分商品库存不足导致下单失败！';
        return false;
      }
    }
    //检测限购 新增cart_ids buddha 金海 20170813
    if(!$this->checkItemQuota($item_ids, $cart_ids, $buynow == 1)){
