...
class ItemSelController extends AddonsController{
  ...
  public function index(){
    ...
    $tpl = I('get.tpl', 'index');
    $active = I('get.active') ? I('get.active') : I('theme') ;
    ...
    switch ($active){
        case 'join' :  //拼团活动，只能添加一个活动产品
            ...
            break;
        case 'joinlist' :  //活動列表
            $ids = M('Join_item')->alias('a')->join('LEFT JOIN __JOIN__ b on b.id=a.join_id')->where('b.status=1')->field('a.item_id')->getField('item_id' ,true);
           $where['id'] = array('IN', implode(',', $ids));
        break;
        case 'seckill':
            $seckillIds = A('Home/Seckill','Event')->getItemIds();
            if($seckillIds){
                $where['id'] = array('not in',$seckillIds);
            }
            break;
    }
    if($items){
    ...
    //添加条件 买送
    if (I('get.param_item')) {
      $where['supplier_id'] = M('Item')->where(array('id'=>I('get.param_item')))->getField('supplier_id');
    }
    //分类过滤
