...
class ItemController extends AdminController {
  ...
  */
  ...
  public function importExcel(){
      $fileName = 'prop';
      $data = addPro($fileName);
      //dump($data);exit;
      //$rst = M('ItemBak')->addAll($data);
      foreach ($data as $key=>$val){
          $prop['cname'] = $val['name'];
          $prop['cid'] = $val['cid_1'];
          $prop['type'] = 'specification';
          $prop['create_time'] = NOW_TIME;
          $rst = M('item_property')->add($prop);
          if($rst){
              foreach ($val['values'] as $k=>$v){
                  $proVal[$k]['code'] = build_unique_code(6, $k);
                  $proVal[$k]['prp_id'] = $rst;
                  $proVal[$k]['option'] = $v;
              }
              M('property_option')->addAll($proVal);
          }
      ...
      if($rst){
          ...
          echo 'error:添加失败！<br/>sql:'.M()->getlastsql();
      }
  }
  ...
  /**
   * 导出到excel文件
   * @author ezhu <max@jipu.com>
   */
  public function export(){
      //定义返回或者操作的字段
      //$field = 'id, number, password, name, amount, balance, FROM_UNIXTIME(create_time, "%Y-%m-%d"), FROM_UNIXTIME(expire_time, "%Y-%m-%d")';
//       $model = M('goods');
//       $field = 'goods_id,cat_id,goods_sn,goods_name,goods_title,goods_number,goods_weight,market_price,shop_price,keywords,goods_desc,goods_thumb,goods_img,original_img';

//       $model = M('attribute');
//       $field = 'attr_name,cat_id,attr_values';
  
      $model = M('goods_attr');
      $field = 'goods_id,attr_id,attr_value,attr_price';
      //定义查询条件
      $map = array();
      //$map['is_delete'] = 0;
  
      //获取礼品卡数据
      $data = $model->where($map)->field($field)->order('goods_attr_id asc')->select();
  
      //文件名
      $filename = "商品-规格表";
  
      //表头
      $headArr = array('商品id', '规格id', '规格选中值','规格价格');
