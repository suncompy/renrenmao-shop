...
class CouponUserController extends AdminController {
  ...
  public function update(){
    ...
    //查询优惠券总数
    $coupon_num = M('Coupon')->where("id = $coupon_id")->getField('num');
    //已使用优惠券数量
     $coupon_used = M('CouponUser')->where("coupon_id = $coupon_id")->select();
     $used_num = count($coupon_used);
     //可用优惠券数量
    $uids = I('post.uids');
    $num = count(explode(',',$uids)) ;
      $coupon_num = $coupon_num - $used_num;
    ($num > $coupon_num) ? $this->error('优惠券发放人数超过优惠券数量！') : null ;
    if(empty($coupon_id)){
