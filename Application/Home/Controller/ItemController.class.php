...
class ItemController extends HomeController{
  ...
  public function detail($id = 0){
    ...
    //赠品处理 下架或者库存不足去掉 buddha 金海
    foreach ($data['send_item'] as $key => $value) {
      $stock = M('Item')->where('id='.$key)->getField('stock');
      if ($stock == 0) {
        unset($data['send_item'][$key]);
      }
    }
    ...
    //秒杀 活动结束或者未开始 buddha 金海 
    $data['seckill'] = A('Item', 'Event')->getSeckill($id);
    if ($data['seckill']['expire_time'] < time() || $data['seckill']['start_time'] > time()) {
      $data['is_doing'] = 0;
    }else{
      $data['is_doing'] = 1;
    }
    ...
    $share = array(
      ...
      'desc' => $data['summary'],
      'img_url' => SITE_URL.'/'. get_image_thumb($data['cover_path'], 100, 100),
      'link' => SITE_URL.U('Item/detail', array('id' => $data['id'], 'sdp_secret' => SHOP_SECRET))
    );
     //获取分销商品推广码
      $str = $_SERVER['PATH_INFO'];
      $code = explode('/',$str)[5];
      $sdp_code = $code ? $code : SHOP_SECRET ;
      //判断登录用户是否是分销商
      if( (C('SDP_PRICE_SHOW') == 1) && $sdp_code ){
          $sdp_fee = ($data['sdp_type'] == 0) ? number_format($data['sdp'],2) : number_format($data['sdp']*$data['price']/100,2);
