...
class AuthManagerController extends AdminController{
  ...
  public function index(){
    $list = $this->lists('AuthGroup',array('module'=>'admin', 'status' => array('gt', -1)),'id asc');
    $list = int_to_string($list);
    foreach($list as &$li){
      $li['pcount'] = D('AuthGroup')->getGroupPcount($li['id']);
    }
  ...
  public function group(){
    ...
    //获取会员等级
    $this->user_group_id = M('User')->getFieldById($uid, 'group_id');
  ...
  public function addToGroup(){
    ...
    if(is_numeric($uid)){
      ...
      // 程序中这块设计不合理
      // if (!M('Supplier')->where(array('supplier_id'=>$uid))->find()) {
      //   $this->error('该用户不是分销商，请先成为分销商再操作！');
      // }
    }
    ...
    if($AuthGroup->addToGroup($uid,$gid)){
      //会员等级更改
      $user_group_id && M('User')->where(array('id'=>$uid))->setField('group_id', $user_group_id);
      //记录行为
      action_log('update_auth_manger', 'authManager', $gid, UID);
