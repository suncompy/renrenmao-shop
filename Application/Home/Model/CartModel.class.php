...
class CartModel extends RelationModel{
   ...
   * @author Max.Yu <max@jipu.com>
   */
  public function lists($map = array(), $field = true, $order = '`update_time` DESC ,`quantity` DESC ,`id` DESC', $limit = '50'){
    ...
    if($list && is_array($list)){
      ...
      foreach($list as $key => &$value){
        ...
        $quantity = min($value['quantity'] , $list[$key]['stock']) ;
  ...
  public function addCart($item_code, $item_id, $quantity, $price, $specifiction, $uid ,$sdp_code){
    ...
    if($isExist){
      ...
      //更新时间 新增 用于排序使用 BUDDHA
      $this->where($where)->setField('update_time',NOW_TIME);
      if($status === false){
        return false;
      }
      //更新统计
      ...
      $data = M('Item')->where('id = '.$item_id)->field('name, number, weight, thumb, supplier_id')->find();
      $data['uid'] = $uid;
