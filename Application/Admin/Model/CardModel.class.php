...
class CardModel extends AdminModel {
  ...
  protected $_validate = array(
    ...
    array('quantity', '/^[1-9]\d*$/', '卡数量格式不合法，必须为大于0的整数', self::VALUE_VALIDATE  , 'regex', self::MODEL_INSERT),
    array('amount', 'require', '卡面值不能为空', self::MUST_VALIDATE, 'regex', self::MODEL_INSERT),
    array('amount', '/^[1-9][0-9]*\d?$/', '卡面值格式不合法，必须为大于0的数字', self::VALUE_VALIDATE  , 'regex', self::MODEL_INSERT),
    array('length', 'require', '卡号长度不能为空', self::MUST_VALIDATE, 'regex', self::MODEL_INSERT),
    array('length', '/^[1-9]\d*$/', '卡号长度格式不合法，必须为大于0的整数', self::VALUE_VALIDATE  , 'regex', self::MODEL_INSERT),
