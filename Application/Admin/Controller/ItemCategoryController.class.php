...
class ItemCategoryController extends AdminController {
  ...
  public function edit($id = null, $pid = 0){
    ...
    if(IS_POST){ //提交表单
      if($_POST['trait'])$_POST['trait']=','.implode(',',$_POST['trait']).',';
  ...
  public function setStatus($ids , $status = 1){
    if(IS_GET && $ids){
      $handle = function( $ids) use( &$handle ){
        $map['pid'] = array('in' , $ids);
        $cats = M('ItemCategory')->where($map)->getField('id' ,true);
        $catss = implode(',' ,$cats);
        if($cats){
          $con = $handle($catss);
          return  $con ? $catss.','.$con : $catss ;
        }else{
          return $catss;
        }
      };
      $cates = $handle($ids); 
      $cates = $cates ? $cates.','.$ids : $ids;
      $cat_map = array(
        'id' => array('in' ,$cates),
        'status' => $status == 1 ? 0 : 1, 
      );
      M('ItemCategory')->where($cat_map)->setField('status' ,$status);
      $item_map = array(
        'cid_1|cid_2|cid_3' => array('in' ,$cates),
        'status' => $status == 1 ? 0 : 1, 
      );
      M('Item')->where($item_map)->setField('status' ,$status);
