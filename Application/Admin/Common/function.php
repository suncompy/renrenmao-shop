 ...
 * @author Max.Yu <max@jipu.com>
 */
function build_unique_code($len=6, $i=''){
...
function importExcel($fileName){
    ...
    $filename = str_replace($p.'Application'.$p.'Admin'.$p.'Common','',dirname(__FILE__)).$p.'Excel'.$p.$fileName.'.xls';
    ...
    for($j=2;$j<=$highestRow;$j++){
        $data[$k]['id'] = $objPHPExcel->getActiveSheet()->getCell("A".$j)->getValue() ? : 0;//获取A列的值
        $data[$k]['cid_1'] = $objPHPExcel->getActiveSheet()->getCell("B".$j)->getValue() ? : 0;//获取B列的值
        $data[$k]['number'] = $objPHPExcel->getActiveSheet()->getCell("C".$j)->getValue() ? : 'ecshop';//获取C列的值
        $data[$k]['name'] = $objPHPExcel->getActiveSheet()->getCell("D".$j)->getValue() ? : '新品';//获取C列的值
        $data[$k]['summary'] = $objPHPExcel->getActiveSheet()->getCell("E".$j)->getValue() ? : '暂无';//获取C列的值
        $data[$k]['stock'] = $objPHPExcel->getActiveSheet()->getCell("F".$j)->getValue() ? : 0;//获取C列的值
        $data[$k]['weight'] = $objPHPExcel->getActiveSheet()->getCell("G".$j)->getValue() ? : 0;//获取C列的值
        $data[$k]['mprice'] = $objPHPExcel->getActiveSheet()->getCell("H".$j)->getValue() ? : 0;//获取C列的值
        $data[$k]['price'] = $objPHPExcel->getActiveSheet()->getCell("I".$j)->getValue() ? : 0;//获取C列的值
        $data[$k]['tag'] = $objPHPExcel->getActiveSheet()->getCell("J".$j)->getValue() ? : '';//获取C列的值
        $data[$k]['intro'] = $objPHPExcel->getActiveSheet()->getCell("K".$j)->getValue() ? : '暂无';//获取C列的值
        $k++;
    }
...
function addPic($fileName){
    ...
    for($j=2;$j<=$highestRow;$j++){
        $map = array();
        $map['id'] = $objPHPExcel->getActiveSheet()->getCell("A".$j)->getValue();//获取A列的值
        $data1 = array();
        $data1['status'] = 1;
        $data1['create_time'] = NOW_TIME;
        $data1['path'] = $objPHPExcel->getActiveSheet()->getCell("L".$j)->getValue() ? : 0;//获取B列的值
        $rst1 = M('picture')->add($data1);
        if($rst1){
            M('item_bak')->where($map)->save(array('thumb'=>$rst1));
            sql().'<br/>\r\n';
        }
    
        $data2 = array();
        $data2['path'] = $objPHPExcel->getActiveSheet()->getCell("M".$j)->getValue() ? : 'ecshop';//获取C列的值
        $data2['create_time'] = NOW_TIME;
        $data2['status'] = 1;
        $rst2 = M('picture')->add($data2);
        if($rst2){
            M('item_bak')->where($map)->save(array('images'=>$rst2));
            sql().'<br/>\r\n';
        }
        $k++;
    }
...
function addPro($fileName){
    ...
    //$a表示列号
    for($j=2;$j<=$highestRow;$j++){
        $data[$k]['name'] = $objPHPExcel->getActiveSheet()->getCell("A".$j)->getValue() ? : 0;//获取A列的值
        $data[$k]['cid_1'] = $objPHPExcel->getActiveSheet()->getCell("B".$j)->getValue() ? : 0;//获取B列的值
        $data[$k]['values'] = parse_config_attr($objPHPExcel->getActiveSheet()->getCell("C".$j)->getValue()) ? : 'ecshop';//获取C列的值
        $k++;
    }
    return $data;
}
...
function addItemProp($fileName){
    ...
    //$a表示列号
    for($j=2;$j<=$highestRow;$j++){
        $data[$k]['item_id'] = $objPHPExcel->getActiveSheet()->getCell("A".$j)->getValue() ? : 0;//获取A列的值
        $data[$k]['pro_id'] = $objPHPExcel->getActiveSheet()->getCell("B".$j)->getValue() ? : 0;//获取B列的值
        $data[$k]['values'] = $objPHPExcel->getActiveSheet()->getCell("C".$j)->getValue() ? : 'ecshop';//获取C列的值
        $data[$k]['price'] = $objPHPExcel->getActiveSheet()->getCell("D".$j)->getValue() ? : 'ecshop';//获取C列的值
        $k++;
    }
    return $data;
}
