...
class JoinController extends AdminController{
    ...
    function setStatuss(){
      ...
      $data['status'] = $status;
      if($status == 1 ){
        $target   = M('Join')->where($map)->getField('item_ids');
        if($target){
          $need    = explode(',' , $target);
          $num     = count($need) ;
          $result  = M('Join')->where('status=1 and id !='.$ids)->getField('item_ids' ,true);
          if($result){
      ...
      M('Join')->where($map)->save($data) ? $this->success('操作成功') : $this->error('操作失败');
    }
    /**
     * 拼团订单展示
