...
class PictureModel extends Model{
  ...
  public function upload($files, $setting, $driver = 'Local', $config = null){
    if(!is_dir($setting['rootPath']) && $driver == 'Local'){
       @mkdir( $setting['rootPath'] , 0755, true);
    }
    /* 上传文件 */
    $setting['callback'] = array($this, 'isFile');  //根据MD5和HASH码检测当前上传的文件是否已存在
    $setting['removeTrash'] = array($this, 'removeTrash');  //清除数据库存在但本地不存在的数据
