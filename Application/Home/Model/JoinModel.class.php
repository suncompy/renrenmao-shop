...
class JoinModel extends Model{
     ...
     * @return array
     */
    public function detail( $item_id , $field=true ,$activity_id = '' ){
        $info = M('Item')->field($field)->find($item_id);
        
        $jMap['a.item_id'] = array('eq',$item_id);
        $jMap['a.stime'] = array('elt',NOW_TIME);
        $jMap['a.etime'] = array('gt',NOW_TIME);
        $jMap['a.status'] = array('eq',1);
        $jMap['b.status'] = array('eq',1);
        if(!empty($activity_id)){
            $jMap['b.id'] = $activity_id ;
        }
        $join = M('join_item')->alias('a')->join(' LEFT JOIN __JOIN__ b on a.join_id=b.id')->where($jMap)->field('a.*')->find();
    ...
    private function getPropertyAll($item_id = null){
        ...
        //拼团后台选择规格
        $propVal = array();
        $field = 'a.spc_code,a.quantity,a.first_price,a.join_price';
        $spc_info = M('join_item_spec')->alias('a')->join('LEFT JOIN __JOIN__ b on b.id=a.activity_id')->where('a.item_id = '.$item_id.' and b.status=1')->field($field)->select();
        if($spc_info){
        ...
        $spc_data = null;
        $stock = 0;
        $lists = M('JoinItemSpec')->alias('a')->join('LEFT JOIN __JOIN__ b on b.id=a.activity_id')->where('a.item_id='.$item_id.' and b.status=1')->field('a.*')->select();
    ...
    public function addHandle(){
        ...
        $time = time();
        $iteMap['a.status']  = array('eq',1);
        $iteMap['a.item_id'] = array('eq',$item_id);
        $iteMap['a.stime']   = array('elt',$time);
        $iteMap['a.etime']   = array('gt',$time);
        $iteMap['b.status']  = array('eq',1);
        $joinItem = M('join_item')->alias('a')->join('LEFT JOIN __JOIN__ b on b.id=a.join_id')->where($iteMap)->find();
    ...
    function getItemStock($item_id, $item_code){
        ...
        $spec_map = array(
                'a.item_id' => $item_id,
                'a.spc_code' => $item_code,
                'b.status'  => 1 
        );
        $check_spec = M('join_item_spec')->alias('a')->join('RIGHT JOIN __JOIN__ b on b.id=a.activity_id')->where($spec_map)->field('a.*')->find();
        if($check_spec){
            ...
            $result = M('join_item')->alias('a')->join('RIGHT JOIN __JOIN__ b on b.id=a.join_id')->where($item_map)->getField('stock');
        }
        return $result;
    }
     ...
     * @param string $joinid 参团的团ID
     * @return array 更新结果
     * @author Max.Yu <max@jipu.com>
     */
    function getItemPrice($item_id, $item_code,$joinid){
        if(empty($item_id) || empty($item_code)){
            return false;
        }
        ...
        $check_spec = M('join_item_spec')->alias('a')->join('LEFT JOIN __JOIN__ b on a.activity_id=b.id')->field('a.*')->where($spec_map)->find();
        if($check_spec){
            $result = $joinid ? $check_spec['join_price'] : $check_spec['first_price'];
        }else{
            $item_map = array(
                'a.item_id' => $item_id,
                'b.status'  => 1 ,
            );
            $priceData = M('join_item')->alias('a')->join('LEFT JOIN __JOIN__ b on a.join_id=b.id')->where($item_map)->field('a.*')->find();
            $result = $joinid ? $priceData['join_price'] : $priceData['first_price'];
        }
