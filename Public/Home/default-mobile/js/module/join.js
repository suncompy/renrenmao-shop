    ...
    specSubmitEvent : function(){
      ...
      $('#Z_spec_submit').on('click', function(){
        if(_self.checkSpecSelect() == 1){
          ...
          // (_self.is_buynow == 1) ? _self.add(1) : _self.add(0);
          //重置数量和已选规格
          // _self.resetNumSpec();
        }else{
          $.ui.error('请您选择规格');
      ...
      if(seckill){
        ...
        if(now > seckill.expire_time){
          ...
          if(now < seckill.start_time){
            //尚未开始
            $('.Z_seckill_time').html('距团购开始有<br/><span class="text-danger day">00</span>天<span class="text-danger hour">00</span>时<span class="text-danger minute">00</span>分<span class="text-danger second">00</span>秒');
            this.countDown(this.unix_to_datetime(seckill.start_time), ".Z_seckill_time");
          }else{
            //进行中
            $('.Z_seckill_time').html('倒计时：<span class="text-danger day">00</span>天<span class="text-danger hour">00</span>时<span class="text-danger minute">00</span>分<span class="text-danger second">00</span>秒');
