...
define('module/cart', function(require, exports, module){
  ...
  var cart = {
    init: function(second_pieces_item){
      var _self = this;
      $('#Z_cart_clear').on('click', function(){
        _self.clear();
      });
      //商品选择
      _self.itemCheck();
      ...
      //更改购物车商品数量 + -
      $('.Z_cart_update').on('click', function(){
        ...
        var option = $(this).data('option'),
            ...
            quantity = $('#Z_item_num_' + item_id + '_' + item_code).val(),
            quantity_to = '';
        quantity_to = (option == 'plus') ? parseInt(quantity) + 1 : parseInt(quantity) - 1;
        _self.update(item_id, item_code, quantity_to, price, stock, second_pieces_item);
      });
      
      //购物车结算
      $('#Z_cart_checkout').on('click', function(){
      ...
      //移出购物车
      $('.Z_cart_remove').on('click', function(){
        var item_code = $(this).attr('data-item_code');
        var item_id = $(this).data('item_id');
        _self.remove(item_id,item_code);
      });
    ...
    secondPiecesPriceInit: function(second_pieces_item){
      ...
      $.each(second_pieces_item, function(i, val){
        var obj = $('.Z_cart_' + val.item_id),
            ...
            num = $('#Z_item_num_' + item_id + '_' + code).val(),
            second_price,
            price = parseFloat($('.Z_cart_' + val.item_id + '_' + code + ' .Z_cart_change').data('price'));
        if(num >= 2){
          second_price = parseFloat(price * val.discount);
          
          $('#Z_item_price_' +val.item_id + '_' + item_code + ' > .Z_item_price').text((second_price + price).toFixed(2));
          $('#Z_second_pieces_' + val.item_id + '_' + item_code).text(val.name).removeClass('hide');
        }
      });
    },
    ...
    setTotal: function(){
      ...
      $('.Z_cart_change').each(function(){
        ...
        if(bool){
          ...
          var _p = $('#Z_item_price_' + item_id + '_' + item_code + ' > .Z_item_price').text();
          total_amount += _p * 1;
          total_num += num;
        }
      });
    ...
    animate: function(image_src, image_top, image_left){
      var cartbox = '<img src="' + image_src + '" id="Z_item_cartbox" class="cart-box-img" style="top: '+ image_top  +'px; left: 50%" width="120" height="120" />';
      var cart_top = $('#Z_add2cart').offset().top; //取得加购物车的高度
      var cart_left = $('#Z_add2cart').offset().left; //取得加购物车的宽度
      $('body').append(cartbox);
      $('body').children('#Z_item_cartbox').animate({
        width: '24px',
        height: '24px',
        top: parseInt(cart_top+10),
        left: parseInt(cart_left + 100),
        opacity: .2
      }, 800, 'swing', function(){
        $(this).remove();
      });
        ...
        success: function(res){
          if(res.status == 1){
            if(is_buynow){
              ...
              var image_top = $('.slider-indicators').offset().top; //开始位置的高度
              var image_left = $('.slider-indicators').offset().left; //开始位置的宽度
    ...
    /*修改购物车商品数量*/
    update: function(item_id, item_code, quantity, price, stock, second_pieces_item){
      var sum_price = '0.00',
          _self = this,
          item_minus = $('#Z_item_box_' + item_id + '_' + item_code).find('.minus'),
          item_plus = $('#Z_item_box_' + item_id + '_' + item_code).find('.plus'),
          //item_id = $('#cart-group-' + item_id + '_' + item_code).data('id'),
      ...
      if(quantity >= 2 && is_second_pieces_discount){
        ...
        $('#Z_item_num_' + item_id + '_' + item_code).val(quantity);
        $('#Z_item_price_' + item_id + '_' + item_code + ' > .Z_item_price').text(sum_price.toFixed(2));
        //$('#J_item_price_' + item_code).html('<em class="yen">&yen;</em> ' + sum_price.toFixed(2));
        $('#Z_second_pieces_' + item_id + '_' + item_code).text(second_pieces_item[item_id]['name']).removeClass('hide');
      }else{
        sum_price = parseFloat(quantity) * price;
        $('#Z_item_num_' + item_id + '_' + item_code).val(quantity);
        $('#Z_item_price_' + item_id + '_' + item_code + ' > .Z_item_price').text(sum_price.toFixed(2));
        $('#Z_second_pieces_' + item_id + '_' + item_code).addClass('hide');
      }
      ...
      $.ajax({
        type: 'POST',
        url: $.U('Cart/update'),
        data: {item_id: item_id, item_code:item_code, quantity: quantity},
        dataType: 'json',
        success: function(res){
          if(res.status == 1){
            _self.setTotal();
    ...
    /*删除购物车内商品*/
    remove: function(item_id,item_code){
      var _self = this;
      $.ui.confirm('确定从购物车中删除吗？', function(){
        $.post($.U('Cart/remove'), {item_id:item_id,item_code:item_code}, function(res){
          if(res.status == 1){
            /*购物车为空时更新提示信息*/
            if(res.total.total_quantity == 0){
              cart.setEmptyTips();
            }
            var supp = $('#cart-group-' + item_id + '-' + item_code).closest('.supplier-box');
            if(supp.find('.cart-group').size() === 1){
              supp.remove();
            }else{
              $('#cart-group-' + item_id + '-' + item_code).remove();
            }
            _self.setTotal();
          }
        }, 'json');
