...
class WechatAuth{
  ...
  private $getJsTicketURL = 'https://api.weixin.qq.com/cgi-bin/ticket';
  
  private $sendTplMsgURL = 'https://api.weixin.qq.com/cgi-bin/message/template/send';
  private $getQrcodeTicketUrl = 'https://api.weixin.qq.com/cgi-bin/qrcode/create';
  ...
  public function __construct($appid, $secret, $token = null){
    if($appid && $secret){
      ...
      throw new \Exception('请您先配置微信app_id和secret！');
    }
  }
  ...
  public function getJsapiTicket(){
    if(!S('jsapiTicket')){
      ...
      if(is_array($info) && $info['access_token']){
      ...
      $info = self::http("{$this->getJsTicketURL}/getticket", $param);
  ...
  public function getAccessToken($type = 'client', $code = null){
    ...
    $cacheName = 'accessToken_'.$type;
    //授权access_token
    if($type == 'code'){
      if(session($cacheName)){
        $s_token = session($cacheName);
        $url = "{$this->oauthApiURL}/auth";
        $arr = array(
          'access_token' => $s_token['access_token'],
          'openid' => $s_token['openid'],
        );
        $token_ = self::http($url, $arr);
        $token = json_decode($token_, true);
        if($token['errcode'] == 0){
          $this->accessToken = $s_token['access_token'];
          return $s_token;
        }else{
      ...
      if(empty($code)){
        if(C('WECHAT_USERINFO_BY_API') == true){
          $access_token = A('User', 'Event')->getAuthAccessTokenByApi();
          $this->accessToken = $access_token['access_token'];
          session($cacheName, $access_token);
          return $access_token;
        }else{
          return array();
        }
    ...
    //缓存不存在、生成
    if(S($cacheName) && $type == 'client'){
      $token = S($cacheName);
      $this->accessToken = $token['access_token'];
      return $token;
    }else{
      $token_ = self::http($url, $param);
      $token = json_decode($token_, true);
      if(is_array($token)){
        if(isset($token['errcode'])){
          ...
          //保存到缓存
          if($type == 'client'){
            S($cacheName, $token, $token['expires_in']);
          }else{
            session($cacheName, $token);
          }
          return $token;
        }
      }else{
  ...
  public function sendTplMsg($data = array()){
    ...
    $res = $this->http($this->sendTplMsgURL, array('access_token' => $token['access_token']), json_encode($data), 'POST');
    return $res;
  }
   ...
   * @author Justin
   */
  public function getQrcodeTicket($scene_id = 0){
    if($scene_id){
      $data = array(
      ...
      $token = $this->getAccessToken();
      $res = $this->http($this->getQrcodeTicketUrl, array('access_token' => $token['access_token']), json_encode($data), 'POST');
      return $res;
    }
  }
