...
define('module/cart', function (require, exports, module){
  ...
  var cart = {
    init: function (second_pieces_item){
      ...
      $('.J_cart_update').on('click', function (){
        var option = $(this).data('option'),
                ...
                quantity = $('#J_item_num_' + item_id+ '_' + item_code).val(),
                quantity_to = '';
        $(this).blur();
      ...
      $('.J_cart_change').on('change keyup', function (){
        ...
        $(this).val(quantity);
        _self.update(item_id, item_code, quantity, price, stock, second_pieces_item);
      });
      ...
      $('#J_cart_checkout').on('click', function (){
        ...
        if(item_checked.length > 0){
          $.each(item_checked, function (i, item){
            item_ids.push($(item).data('car_id'));
          });
    ...
    secondPiecesPriceInit: function(second_pieces_item){
      ...
      $.each(second_pieces_item, function(i, val){
        var obj = $('.J_cart_' + val.item_id),
            code = obj.data('code'),
            num = $('#J_item_num_' + val.item_id).val(),
            second_price,
            price = parseFloat($('.J_cart_' + val.item_id + ' .J_cart_change').data('price'));
        if(num >= 2){
          second_price = parseFloat(price * val.discount);
          $('#J_item_price_' + val.item_id + '_' + code + ' > .J_item_price').text((second_price + price).toFixed(2));
          $('#J_second_pieces_' + val.item_id + '_' + code ).text(val.name).removeClass('hide');
        }
      });
    },
    ...
    setTotal: function (){
      ...
      $('.J_cart_change').each(function (){
        ...
        if(bool){
          ...
          total_amount += parseFloat($('#J_item_price_' + item_id + '_' + item_code + ' > .J_item_price').text());
    ...
    update: function (item_id, item_code, quantity, price, stock, second_pieces_item){
      var sum_price = '0.00',
          _self = this,
          item_minus = $('#J_item_box_' + item_id + '_' + item_code).find('.minus'),
          item_plus = $('#J_item_box_' + item_id + '_' + item_code).find('.plus'),
          item_id = $('#J_item_box_' + item_id + '_' + item_code).closest('dd').data('id'),
      ...
      if(quantity >= stock){
        quantity = stock;
        var gt_obj = $('#J_item_box_' + item_id + '_' + item_code).closest('dd').find('.J_gt_freenum_tip');
        if(gt_obj.size() === 1){
          gt_obj.slideUp();
        }
      }
      ...
      if(quantity >= 2 && is_second_pieces_discount){
        ...
        $('#J_item_num_' + item_id + '_' + item_code).val(quantity);
        $('#J_item_price_' + item_id + '_' + item_code +' > .J_item_price').text(sum_price.toFixed(2));
        //$('#J_item_price_' + item_code).html('<em class="yen">&yen;</em> ' + sum_price.toFixed(2));
        $('#J_second_pieces_' + item_id + '_' + item_code).text(second_pieces_item[item_id]['name']).removeClass('hide');
      }else{
        sum_price = parseFloat(quantity) * price;
        $('#J_item_num_' + item_id + '_' + item_code).val(quantity);
        $('#J_item_price_' + item_id + '_' + item_code +' > .J_item_price').text(sum_price.toFixed(2));
        $('#J_second_pieces_' + item_id + '_' + item_code).addClass('hide');
      }
        ...
        $.ajax({
          ...
          success: function (res){
            if(res.status == 1){
              ...
              var supp = $('#J_cart_' + item_id + '_' + item_code).closest('.supplier-box');
              if(supp.find('.item').size() === 1){
                supp.remove();
              }else{
                $('#J_cart_' + item_id + '_' + item_code).remove();
              }
              _self.setTotal();
              //更新导航栏购物车商品数量
              $('#J_cart_quantity').html(res.total.total_num);
